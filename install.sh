...
# Builds engine + gateway + admin, installs to /opt/forge, and (if systemd is available) starts the gateway.
...
install -m 755 engine/target/release/forge-engine engine/forge-engine.new && mv -f engine/forge-engine.new engine/forge-engine   # atomic replace, works while the old binary runs
echo "== build gateway"
...
if [ ! -f gateway.env ]; then
  ...
  sed -i "s#/opt/forge#$DEST#g; s#change-me-long-random#$(head -c 24 /dev/urandom | base64 | tr -d '/+=')#" gateway.env
  echo "== wrote $DEST/gateway.env (ADMIN_TOKEN generated — see inside)"
fi

if command -v systemctl >/dev/null && [ -d /run/systemd/system ]; then
  $SUDO cp deploy/forge-gateway.service /etc/systemd/system/forge-gateway.service
  $SUDO sed -i "s#/opt/forge#$DEST#g; s#^User=forge#User=$(id -un)#" /etc/systemd/system/forge-gateway.service
  $SUDO systemctl daemon-reload && $SUDO systemctl enable --now forge-gateway
  echo "== gateway running as a service:  systemctl status forge-gateway"
else
