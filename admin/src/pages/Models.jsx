...
function ModelCard({ m, busy, action, save, models }) {
  ...
  const color = { running: 'ok', ready: '', downloading: 'muted', error: 'bad' }[m.status] || ''
    ...
    <div className="card" style={{ marginBottom: 12 }}>
      <div className="row" style={{ marginBottom: 6 }}>
        <div style={{ flex: 1 }}>
          ...
          <div className="sub">
            <span className={color}>{m.status}{m.port ? ` on :${m.port}` : ''}</span>
        ...
        {m.status === 'running' && <button className="primary" style={{ background: 'var(--red)' }} onClick={() => action(m.id, 'stop')} disabled={!!busy}>stop</button>}
