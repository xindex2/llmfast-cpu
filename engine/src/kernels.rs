...
fn tile_rows(rows: &[f32], r0: usize, r1: usize, xs: &[f32], m: usize, n: usize, k: usize, yp: SendPtr) {
        // Row group outermost, all m columns inside it. Blocking this loop over columns to keep
        // the activation slice inside L2 was tried and measured on the 2x10-core Xeon it was
        // designed for: 21.4 tok/s prefill unblocked against 17.3 / 16.1 / 15.3 at 128 / 64 /
        // 256 KB budgets. Keeping the four rows hot across every column beats any saving on
        // activation traffic, so the obvious order is also the right one -- don't re-try it.
        let mut i = 0;
        while i + 4 <= r1 - r0 {
